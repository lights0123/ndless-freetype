...
extern "C" fn alloc_library(_memory: ffi::FT_Memory, size: c_long) -> *mut c_void {
	unsafe { ndless_sys::malloc(size as size_t) }
}

extern "C" fn free_library(_memory: ffi::FT_Memory, block: *mut c_void) {
	unsafe { ndless_sys::free(block) }
}
...
extern "C" fn realloc_library(
	...
	unsafe { ndless_sys::realloc(block, new_size as size_t) }
}
...
impl Library {
	...
	pub fn new_face(&self, path: impl Into<String>, face_index: isize) -> FtResult<Face> {
		...
		let path = CString::new(path.into()).ok().ok_or(Error::InvalidPath)?;
		let err = unsafe {
			ffi::FT_New_Face(
				self.raw,
